...
)  # Wall-clock time of VQE run only, in hours. In the case of a restart, it measures the time from the beginning of the first run to the end of the last run, even if there were large times in between.
########################
...
if cmd_args.cost_fn == "energy":
    ...
    run_args.inf_VQE = _HVQE.infidelity_from_parameters(
        init_reg,
        run_args.layers,
        run_args.n,
...
if cmd_args.cost_fn == "infidelity":
    ...
    run_args.E_VQE = _HVQE.Heisenberg_energy_from_parameters(
...
vqe_out.opt_parameters = (
    vqe_out.opt_parameters.array.tolist()
)  # Convert for printing and storing.
...
_HVQE.plot_VQE_data(
    cmd_args.path, "infidelity", cmd_args.par_multiplicity, gates_per_cycle
)
_HVQE.plot_VQE_data(
    cmd_args.path, "wall_clock", cmd_args.par_multiplicity, gates_per_cycle
)
