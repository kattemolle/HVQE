...
def get_command_line_input():
    parser = argparse.ArgumentParser(
        description="Run a VQE using the quantum emulator qem."
    )
    ...
    parser.add_argument(
        "path",
        type=str,
        help="The path to the folder in which the file graph_input.txt is located. Output files will be placed in the same folder.",
    )
    parser.add_argument(
        "n_par", type=int, help="The number of parameters to be used in the VQE."
    )
    parser.add_argument(
        "par_multiplicity",
        type=int,
        help="The parameter multiplicity; the number of gates that share the same parameter. Parameters are given to the gates in the order in which edges apear in the (flattend version of) the list 'layers_input', which should be defined in the file path/graph_input.txt",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="energy",
        help="The cost function that the VQE tries to minimize. Should be 'energy' or 'infidelity'. Note that on a quantum computer it would be impractical to use the infidelity as a cost function.",
    )
    parser.add_argument(
        "--temperature",
        "-T",
        type=float,
        default=1.0,
        help="Temperature for the metropolis creterion in the scipy basinhopping routine.",
    )
    parser.add_argument(
        "--stepsize",
        "-s",
        type=float,
        default=1.0,
        help="The max stepsize of random displacement per parameter after each local optimization in the scipy basinhopping routine.",
    )
    parser.add_argument(
        "--init_par",
        type=float,
        nargs="+",
        default=None,
        help="A list of initial parameters from which the basinhopping routine starts. Use as: --init_par par1 par2 par3 ... . If no --init_par is given, random parameters will be chosen in a specific interval. These are exported to output.txt at the end of the script for reproducibility. See VQE.run_VQE for more info.",
    )
    parser.add_argument(
        "--dump_interval",
        type=int,
        default=None,
        help="Dump the state of the program to path/dump.dat every dump_interval function calls.",
    )
    ...
    for line_nr, line in enumerate(_data):
        if line != "\n":
            try:
                ...
                print(
                    "WARNING: corruption found in line {} of {}".format(
                        line_nr + 2, path
                    )
                )
...
def optimal_line(data, n_par, n_iter, par_multiplicity, load_pe=0, run_pe=0):
    ...
    _data = [
        ...
        and line[0]["n_iter"] == n_iter
        and line[0]["par_multiplicity"] == par_multiplicity
    ]
    ...
    if len(_data) == 0:
        print(
            "optimal_line: no entries found for n_par={}, n_iter={}, load_pe={}, run_pe={} in data".format(
                n_par, n_iter, load_pe, run_pe
            )
        )
        return None
    else:
...
def Heisenberg_energy_from_parameters(
...
def noisy_Heisenberg_energy_and_infidelity_from_parameters(
    complete_graph,
    init_reg,
    layers,
    n,
    ...
    assert (
        par_multiplicity == 1
    ), "Error functionality is only available for One parameter Per Gate (OPG), i.e. par_multiplicity=1"
    reg = qem.EmptyReg(n)
    reg.psi = init_reg.psi
...
def infidelity_from_parameters(
...
def run_VQE(cmd_args, run_args, init_reg, gs_reg):
    ...
    def calc_cost(parameters):
        ...
        if cmd_args.cost_fn == "energy":
            cost = Heisenberg_energy_from_parameters(
                run_args.complete_graph,
                init_reg,
                run_args.layers,
                run_args.n,
                cmd_args.par_multiplicity,
                parameters,
            )
        elif cmd_args.cost_fn == "infidelity":
            cost = infidelity_from_parameters(
                init_reg,
                run_args.layers,
                run_args.n,
                cmd_args.par_multiplicity,
    ...
    if cmd_args.init_par is None:
        ...
        assert (
            len(cmd_args.init_par) == cmd_args.n_par
        ), "List of initial parameters must be of length n_par."
        vqe_out.init_par = numpy.array(cmd_args.init_par)
    if (
        cmd_args.n_par == 0
    ):  # If there is no circuit, just output the energy of the init state.
        if cmd_args.cost_fn == "energy":
            vqe_out.cost_VQE = Heisenberg_energy_from_parameters(
                run_args.complete_graph,
                init_reg,
                run_args.layers,
                run_args.n,
        ...
        sol = scipy.optimize.basinhopping(
            calc_cost,
            vqe_out.init_par,
            stepsize=cmd_args.stepsize,
            minimizer_kwargs={"jac": True},
            niter=cmd_args.n_iter,
            interval=25,
            callback=callback,
            T=cmd_args.temperature,
        )
...
def plot_VQE_data(path, fn, par_multiplicity, gates_per_cycle):
    ...
    for n_iter in n_iter_set:
        ...
        n_iter_array.sort(
            key=lambda x: x[0]["n_par"]
        )  # Put datapoints in order of increasing number of parameters.
    ...
    for n_iter_class in data:
        ...
        if fn == "energy":
            ...
            E_VQE_list = [
                -(E_VQE - E[0]) / E[0] for E_VQE in E_VQE_list
            ]  # The relative error in the energy is going to be plotted.
    ...
    if fn == "energy":
        ax.axhline(
            y=-(E[1] - E[0]) / E[0]
        )  # Plot a horizontal line at the first excited state.
        ax.axhline(
            y=-(E[1] - E[0]) / E[0] / 2, ls="--"
        )  # Plot a horizontal dashed line halfway the ground state and the first excited state.
