...
def check_noisy_data(
    data, noiseless_data, gates_per_cycle, p_max, occurences
):  # Check if all noisy points were obtained.
    ...
    for idx, lst in enumerate([pe2list, pe3list, pe4list, pe5list]):
        for n_par in range(
            gates_per_cycle, (p_max + 1) * gates_per_cycle, gates_per_cycle
        ):
...
def inf_plotter(
    data, load_pe, run_pe, gates_per_cycle, p_max, ax, error_bars=False, skip=False
):
    plt_data = []
    for n_par in range(0, (p_max + 1) * gates_per_cycle, gates_per_cycle):
        p = n_par / gates_per_cycle
...
def plotter(
    ...
    l1 = ax[1].legend(
        [bl_ln], ["ana."], loc="lower center", bbox_to_anchor=(0.36, 0.0, 0.36, 0.5)
    )
    ax[1].add_artist(l0)
    ax[1].add_artist(l1)
...
if __name__ == "__main__":
    ...
    fname = "noisy_KVQE_G"
    plotter(
        "../data/kagome/open/on_grid/opg/energy/2x2/", out_prefix, fname, 30, 16, 10
    )

    fname = "noisy_KVQE_K"
    plotter(
        "../data/kagome/periodic/own_graph/opg/energy/3x2/",
        out_prefix,
        fname,
        36,
        37,
    ...
    fname = "noisy_CVQE"
    plotter(
        "../data/chain/periodic/own_graph/opg/energy/20/", out_prefix, fname, 20, 11, 32
    )
