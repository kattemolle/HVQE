...
def plot_VQE_data(ax,cost_fn,plot_fn,prefix):
    ...
    if plot_fn=='energy':
        ...
        for i in range(1,n_exited+1):
            ax.axhline(y=-(E[i]-E[0])/E[0],alpha=1,color=colors[1],zorder=-100) # Plot a horizontal line at the ith excited state.
...
def plot_inset(in_what_ax):
    ...
    ax = inset_axes(in_what_ax, width=.75, height=.75)
    ...
    # sort the lists
    together=[[p_set[i],min_E_list[i],min_inf_list[i],inf_list[i]] for i in range(len(p_set))]
    together=sorted(together,key=lambda x: x[0])
    p_set=[together[i][0] for i in range(len(p_set))]
    min_E_list=[together[i][1] for i in range(len(p_set))]
    min_inf_list=[together[i][2] for i in range(len(p_set))]
    ...
    for i in range(till):
        ax.scatter([p_set[i]]*len(inf_list[i]),inf_list[i],alpha=.1,edgecolors='none',c=colors[0])
    # On the x-axis, put the number of cycles rather than the number of parameters.
    ax.set_xlabel('$p$')
...
plot_VQE_data(axs[1],'energy','infidelity',prefix)
fig.tight_layout()
fig.savefig('../../Postdoc/Papers/Variational quantum eigensolver for the Heisenberg antiferromagnet on the kagome lattice/v3/chain_plot.pdf',bbox_inches='tight',pad_inches=0.005)
...
plot_VQE_data(axs[1],'energy','infidelity',prefix)
fig.tight_layout()
fig.savefig('../../Postdoc/Papers/Variational quantum eigensolver for the Heisenberg antiferromagnet on the kagome lattice/v3/kagome_on_grid_plot.pdf',bbox_inches='tight',pad_inches=0.005)
