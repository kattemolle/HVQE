...
for th in [10**-m for m in range(1,8,1)]:
    ...
    for n in range(4,24,2):
        data=_HVQE.load_data('../data/chain/periodic/own_graph/opg/energy/{}/output.txt'.format(n))
...
for i in range(6,-1,-1):
    ax[0].plot(lst[i][0],lst[i][1],'-o',label='$10^{-{'+str(i+1)+'}}$',alpha=.7,markeredgecolor='none',markersize=5,solid_capstyle='round')
...
for i in range(0,len(lst3)):
    ax[1].plot(lst3[i][0],lst3[i][1],'-o',label=str(i*2+4),alpha=.7,markeredgecolor='none',markersize=5,solid_capstyle='round')
...
l1 = ax[1].legend([bl_ln],['$\sim x^2$'],loc='upper left',bbox_to_anchor=(1,.2))
ax[1].add_artist(l0)
ax[1].add_artist(l1)
