...
def get_command_line_input():
    ...
    parser.add_argument('noise_type', type=str,help='Noise type: depol or bitflip. In earlier runs no noise type was specified. Then it is depol.')
    parser.add_argument('pe',type=float,help='Probability of depolarizing error')
    parser.add_argument('line',type=int,help='Load noiseless data from non-trivial line in output.txt with number LINE')
    parser.add_argument('ent_shots',type=int,help='Expected number of shots where an error occurs. (Expected non-trivial shots)')
...
def load_data_line(line):
    ...
    with open('output.txt','r') as f:
        ...
        _data=f.readlines()
    data=[]
    for line_nr,_line in enumerate(_data):
        if _line !='\n':
            try:
                new_line=eval(_line.strip())
                data.append(new_line)
            except:
                print('WARNING: corruption found in line {} of output.txt'.format(line_nr+2))

    print('Loaded line {} of {}'.format(line,len(data)))

    data_line=data[line]
    assert data_line[0]['par_multiplicity']==1, 'Noisy simulation only available for a parameter multiplicity of 1'
...
if __name__=='__main__':
    ...
    out=Name()
    data_line=load_data_line(cmd_args.line)
    ...
    # Compute the numner of shots so that there will be an expected nt_shots non-trivial shots
    layers=data_line[1]['layers']
    edges=[edge for layer in layers for edge in layer]
    d=data_line[0]['n_par']/len(edges)*len(layers)
    assert d%1==0
    d=int(d)
    out.shots=int(cmd_args.ent_shots/(1-(1-cmd_args.pe)**(data_line[1]['n']*(d+1))))
    ...
    with ch.no_backprop_mode():
        ...
        nntr_E_lst=[] # Noisy non-trivial list of energies. Contains the shots where a noise event occured only.
        nntr_inf_lst=[]
        for s in range(out.shots):
            fn_args=[data_line[1]['complete_graph'],
                     init_reg,
                     data_line[1]['layers'],
                     data_line[1]['n'],
                     ...
                     cmd_args.noise_type,
                     gs_reg]
            noisy_E,noisy_inf=_HVQE.noisy_Heisenberg_energy_and_infidelity_from_parameters(*fn_args)
