...
def load_data_line(line):
    ...
    data_line = data[line]
    assert (
        data_line[0]["par_multiplicity"] == 1
    ), "Noisy simulation only available for a parameter multiplicity of 1"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "In environment_GPU.yml, scipy is not updated to a version that includes bootstrap.. Try updating to en environment with scipy>=1.7.2 (conda update --all) or switching to environment_no_GPU.yml"
        )

    ## Energy
    out.noisy_E_mean = np.mean(comp_E_lst)
    bsres = stats.bootstrap([comp_E_lst], np.mean, batch=1)  # Bootstrap result
    out.bootstrap_E_CI_low = (
        bsres.confidence_interval.low
    )  # Confidence level of the mean of E.
    ...
    if cmd_args.noise_type == "depol":
        if not os.path.exists("noisy_output.txt"):
            ...
            f.write(
                "### Noiseless and noisy output of the VQE is written to this file.\n"
            )
        ...
        if not os.path.exists("noisy_bitflip_output.txt"):
            ...
            f.write(
                "### Noiseless and noisy bitflip output of the VQE is written to this file.\n"
            )
